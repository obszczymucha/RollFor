...
local function Report(text, silent)
  ...
  if IsInRaid() and not _silent then
    SendChatMessage(text, "RAID")
  elseif IsInGroup() and not _silent then
...
local function ProcessData(data)
  ...
  local entries = data.softreserves
  ...
  for i = 1, #entries do
    ...
    for j = 1, #items do
      ...
      if not softResItems[id] then
        softResItems[id] = {}
      end

      if not M:TableContainsValue(softResItems[id], entry.name) then table.insert(softResItems[id], entry.name) end
      if not softResPlayerNameOverrides[entry.name] then softResPlayerNameOverrides[entry.name] = { ["override"] = entry
          .name, ["similarity"] = 0 } end
    end
  end
...
local function ShowSoftRes(args)
  ...
  local colorize = function(player) return format("|cff%s%s|r", M:IsPlayerInMyGroup(player) and "ffffff" or "ff2f2f",
    player) end
...
local function SoftResPlayerNameOverride(args)
  ...
  if target and IsPlayerAlreadyOverridingAName(target) then
    ...
    M:PrettyPrint(format("Player |cffff2f2f%s|r is already overriding |cffff9f69%s|r!", target,
...
local function ShowSoftResPlayerNameUnoverrideOptions()
  ...
  for i = 1, #players do
    ...
    local nextPlayer = format("[|cffff9f69%d|r]:|cff%s%s|r (|cff%s%s|r)", i, color(player), player, color(overrider),
      overrider)
...
local function ShowSoftResPassOptions()
  ...
  for i = 1, #softResPassOptions do
    ...
    local nextPlayer = format("[|cffff9f69%d|r]:|cff%s%s|r", i, M:IsPlayerInMyGroup(player) and "ffffff" or "ff2f2f",
      player)
...
local function SoftResPass(args)
  ...
  for i in (args):gmatch "(%d+)" do
    ...
    if index > 0 and index <= count then
      ...
      M:PrettyPrint(format("|cff%s%s|r is not soft-ressing.", M:IsPlayerInMyGroup(player) and "ff9f69" or "ff2f2f",
        player))
    end
  end
...
local function SoftResUnpass(args)
  ...
  for i in (args):gmatch "(%d+)" do
    ...
    if index > 0 and index <= count then
      ...
      M:PrettyPrint(format("|cff%s%s|r is soft-ressing now.", M:IsPlayerInMyGroup(player) and "ff2f2f" or "ff9f69",
        player))
    end
  end
...
local function ThereWasATie(topRoll, topRollers)
  ...
  M:PrettyPrint(format("The %shighest %sroll was %d by %s.", not reRolling and winnerCount > 0 and "next " or "",
    reRolling and "re-" or "", topRoll, topRollersStrColored, itemOnRoll), M:GetGroupChatType())
  SendChatMessage(format("The %shighest %sroll was %d by %s.", not reRolling and winnerCount > 0 and "next " or "",
    reRolling and "re-" or "", topRoll, topRollersStr, itemOnRoll), M:GetGroupChatType())
  rolls = {}
  rollers = topRollers
  reRolling = true
  rolling = true
  ModUi:ScheduleTimer(function() SendChatMessage(format("%s re-roll for %s now.", topRollersStr, itemOnRoll),
    M:GetGroupChatType()) end, 2.5)
end
...
local function PrintWinner(roll, players)
  ...
  M:PrettyPrint(format("%s %srolled the %shighest (%s) for %s.", M:TableToCommifiedPrettyString(players, f),
    reRolling and "re-" or "", not reRolling and winnerCount > 0 and "next " or "", f(roll), itemOnRoll))
  SendChatMessage(format("%s %srolled the %shighest (%d) for %s.", M:TableToCommifiedPrettyString(players),
    reRolling and "re-" or "", not reRolling and winnerCount > 0 and "next " or "", roll, itemOnRoll),
    M:GetGroupChatType())
end

local function PrintRollingComplete()
  local itemsLeft = itemOnRollCount > 0 and
      format(" (%d item%s left)", itemOnRollCount, itemOnRollCount > 1 and "s" or "") or ""
...
local function RollFor(whoCanRoll, count, item, seconds, info, softRes)
  ...
  if softResCount > 0 and softResCount <= count then
    M:PrettyPrint(format("%s soft-ressed by %s.",
      softResCount < count and format("%dx%s out of %d", softResCount, item, count) or item,
      M:TableToCommifiedPrettyString(softRes, highlight)))
    SendChatMessage(format("%s soft-ressed by %s.",
      softResCount < count and format("%dx%s out of %d", softResCount, item, count) or item,
      M:TableToCommifiedPrettyString(softRes)), GetRollAnnouncementChatType())
  ...
  SendChatMessage(format("Roll for %s%s%s%s", itemOnRollCount > 1 and format("%dx", itemOnRollCount) or "", item,
    (not info or info == "") and "." or format(" %s.", info), countInfo), GetRollAnnouncementChatType())
  reRolling = false
  rolling = true
...
local function IncludeReservedRolls(itemId)
  local reservedByPlayers = FilterAbsentPlayers(FilterSoftResPassingPlayers(OverridePlayerNames(M:CloneTable(softResItems
    [itemId])))) -- If someone has been overriden
  local rollers = reservedByPlayers and M:CountElements(reservedByPlayers) > 0 and reservedByPlayers or
      M:GetAllPlayersInMyGroup()
...
local function CheckSoftRes(silent)
  ...
  elseif #absentPlayersWhoSoftRessed == 0 then
    ...
    for player, override in pairs(overrides) do
      ...
      M:PrettyPrint(format("Auto-matched %s to %s (%s similarity).", highlight(player), highlight(overriddenName),
        similarity))
...
local function GetAllPlayers()
  return M:GetAllPlayersInMyGroup()
end
...
local function OnFirstEnterWorld()
  ...
  SLASH_ARF1 = "/arf"
  SlashCmdList["ARF"] = function(args) ProcessRollForSlashCommand(args, "/arf", GetAllPlayers) end
  SLASH_CR1 = "/cr"
...
local function Init()
  M.PrettyPrint = function(mod, message) ChatFrame1:AddMessage(format("|cff209ff9RollFor|r: %s", message)) end
end
