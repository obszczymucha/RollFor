  ...
  for _, player in pairs( playersInGroupWhoDidNotSoftRes ) do
    ...
    for _, candidate in pairs( playersNotInGroupWhoSoftRessed ) do
      local prediction = { [ "candidate" ] = candidate, [ "similarity" ] = StringSimilarity( player, candidate ),
        [ "levenshtein" ] = Levenshtein2( player, candidate ) }
...
function AssignPredictions( predictions, threshold )
  ...
  for player, prediction in pairs( predictions ) do
    ...
    if topCandidate[ "similarity" ] >= threshold then
      results[ player ] = { [ "override" ] = topCandidate[ "candidate" ],
...
test( "Should order predictions from highest to lowest for Ebonymaw", function()
  ...
  return eq( p1[ "candidate" ], "Ebonymaw" ) and eq( p1[ "similarity" ], 1.0 ) and eq( p1[ "levenshtein" ], 0 ) and
      eq( p2[ "candidate" ], "Ebonynaw" ) and eq( p2[ "similarity" ], 0.72886898685566 ) and eq( p2[ "levenshtein" ], 1 ) and
      eq( p3[ "candidate" ], "Ébonyma" ) and eq( p3[ "similarity" ], 0.75 ) and eq( p3[ "levenshtein" ], 3 ) and
      eq( p4[ "candidate" ], "Ébonynaw" ) and eq( p4[ "similarity" ], 0.58823529411765 ) and eq( p4[ "levenshtein" ], 9 ) and
      eq( p5[ "candidate" ], "Ébonymaws" ) and eq( p5[ "similarity" ], 0.77777777777778 ) and eq( p5[ "levenshtein" ], 10 )
end )
...
test( "Should order predictions from highest to lowest for Cyakblayt", function()
  ...
  return eq( p1[ "candidate" ], "Cykablyat" ) and eq( p1[ "similarity" ], 0.24845199749998 ) and eq( p1[ "levenshtein" ], 4 ) and
      eq( p2[ "candidate" ], "Aisholay" ) and eq( p2[ "similarity" ], 0.35294117647059 ) and eq( p2[ "levenshtein" ], 9 )
end )
...
test( "Should show similarity between Vtank and Vivo", function()
  ...
  return eq( p1[ "candidate" ], "Vtank" ) and eq( p1[ "similarity" ], 1.0 ) and eq( p1[ "levenshtein" ], 0 ) and
      eq( p2[ "candidate" ], "Vivo" ) and eq( p2[ "similarity" ], 0.22222222222222 ) and eq( p2[ "levenshtein" ], 5 )
end )
